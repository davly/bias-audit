...
func TestNewRunAnchor_PayloadShape(t *testing.T) {
	...
	v := NewRunAnchor("cadence-check", 3, digest, sealedAt)

	wantHex := hex.EncodeToString(digest[:])
	if v.Substrate != "flagships/bias-audit/audit-ledger" {
	...
	if v.Location != "flagships/bias-audit/audit-ledger@cadence-check" {
	...
	if v2 := NewRunAnchor("cadence-check", 3, digest, sealedAt); v2 != v {
...
func TestSeal_Success(t *testing.T) {
	digest := fixedDigest()
	want := NewRunAnchor("cadence-check", 1, digest, time.Date(2026, 6, 11, 9, 30, 0, 0, time.UTC))
...
func TestSeal_Non201(t *testing.T) {
	...
	_, err := NewClient(srv.URL).Seal(NewRunAnchor("cadence-check", 1, fixedDigest(), time.Now().UTC()))
	if err == nil {
...
func TestAnchorRun_Disabled(t *testing.T) {
	for _, url := range []string{"", "   "} {
		...
		rcpt, anchored, err := AnchorRun(url, "cadence-check", fc, time.Now().UTC())
		if err != nil {
...
func TestAnchorRun_SelfCheckFailureSealsNothing(t *testing.T) {
	...
	_, anchored, err := AnchorRun(srv.URL, "cadence-check", fc, time.Now().UTC())
	if err == nil {
...
func TestAnchorRun_Success(t *testing.T) {
	...
	rcpt, anchored, err := AnchorRun(srv.URL, "cadence-check", fc, time.Now().UTC())
	if err != nil {
	...
	if _, _, err := AnchorRun(srv.URL, "cadence-check", fc, time.Now().UTC()); err != nil {

...
// bias-audit run-ledger into the Stele verified-trust spine
// (infrastructure/stele) via POST /v1/verdicts.
//
// 2026-06-11 second flagship consumer wire to the spine (Pattern A,
// ported from the ofgemwatch exemplar; R145.B sibling-branch ship —
// see internal/firewall/ TestR145B_SteleAnchorConfinement). This is a
// deliberately LOCAL mirror of the spine wire-contract — the same
// pattern as nexus's internal stele_client — because bias-audit's
// go.mod is zero-require from inception per R174 and MUST NOT import
// the stele Go module.
//
// OFF BY DEFAULT: the CLI anchors only when BIASAUDIT_STELE_URL is
...
//     so explicitly: the oracle is bias-audit's own ledger verifier,
...
//     auditledger.(*Ledger).SelfCheck (JSONL of Entry in append
//     order).
...
// entirely. This is the ONE env read bias-audit performs; the R145.B
// confinement pin in internal/firewall/ pins it to a single call site
// in cmd/bias-audit.
const EnvURL = "BIASAUDIT_STELE_URL"

// Substrate is the spine substrate identifier for bias-audit's
// audit-ledger anchors.
const Substrate = "flagships/bias-audit/audit-ledger"

// OracleStrengthSelfCheck is the honest oracle-strength label for an
// audit-ledger anchor: the verifier is bias-audit itself.
...
func NewRunAnchor(command string, entries int, ledgerDigest [sha256.Size]byte, sealedAt time.Time) Verdict {
	...
	return Verdict{
		...
		Evidence: fmt.Sprintf(
			"run self-check: %d entries, ledger digest %s; oracle is bias-audit's own ledger verifier (self-check, NOT an independent gauntlet)",
